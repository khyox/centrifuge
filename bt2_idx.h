...
#define Ebwt_INITS                                          \
	...
	Ebwt(const string &in,
		 ...
		 bool skipLoading = false) : Ebwt_INITS
	{
		...
		if (verbose || startVerbose)
			cerr << "Opening \"" << in3Str.c_str() << "\"" << endl;
		...
		if (1)
		{ //! this->_offw) { // Skip if there are many sequences (e.g. >64K).
			...
			for (map<uint64_t, uint64_t>::const_iterator size_itr = _size.begin(); size_itr != _size.end(); ++size_itr)
			{
				...
				if (!((size_node.rank == RANK_UNKNOWN && size_node.leaf) || tax_rank_num[size_node.rank] < tax_rank_num[RANK_SPECIES]) || size_node.parent_tid == c_tid)
					continue;
		...
		if (verbose || startVerbose)
			cerr << "Opening \"" << in4Str.c_str() << "\"" << endl;
			...
			if (name_table_fname != "")
			{
				...
				if (table_file.is_open())
				{
					...
					while (!table_file.eof())
					{
						...
						if (line[0] == 0 || line[0] == '#')
							continue;
						if (!strstr(line, "scientific name"))
							continue;
						...
						if (tree_color.find(tid) == tree_color.end())
							continue;
						string temp;
						while (true)
						{
	...
	template <typename TStr>
	static TStr join(EList<FileBuf *> &l, EList<RefRecord> &szs, index_t sztot, const RefReadInParams &refparams, uint32_t seed);
	template <typename TStr>
	void joinToDisk(EList<FileBuf *> &l, EList<RefRecord> &szs, index_t sztot, const RefReadInParams &refparams, TStr &ret, ostream &out1, ostream &out2);
	template <typename TStr>
	void buildToDisk(InorderBlockwiseSA<TStr> &sa, const TStr &s, ostream &out1, ostream &out2, ostream *saOut, ostream *bwtOut, ostream &out4, const EList<RefRecord> &szs, int kmer_size);
	...
	bool &straddled) const
{
	...
	while (true)
	{
		...
		{
			if (upper > off)
			{ // not last element, but it's within
